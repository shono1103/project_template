---
name: list-task
description: job/ 配下のタスクを状態別 (progress / todo / done) に一覧表示する。案件名を指定するとその案件だけに絞り込む。「タスク一覧」「今なにが進行中か」「残っているタスク」「<案件名> のタスク」のように、タスクの状況を知りたいときに使う。参照のみで状態は変更しない。
---
...
```sh
J="job/<案件名>/task"
for s in progress todo done; do
  find "$J/$s" -mindepth 1 -maxdepth 1 -not -name '.gitkeep' -exec basename {} .md \; | sort
done
```

表示順は **progress → todo → done** とする (今動いているものを先頭に置く)。
...
合計: progress 1 / todo 3 / done 3
```

`done` も表示する指定があった場合は、件数の代わりに progress・todo と同じ形式で並べる。
